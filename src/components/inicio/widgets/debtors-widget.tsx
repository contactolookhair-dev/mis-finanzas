    ...
    <SurfaceCard variant="soft" padding="sm" className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {size === "compact" ? null : (
            <p className="mt-1 text-sm text-slate-600">Cobros pendientes y estado de cuotas.</p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white/80 p-3">
          ...
          <div className="mt-2 flex items-baseline justify-between gap-3">
            ...
            <p className="text-sm font-semibold text-slate-600">
              Total: {loading ? "…" : formatCurrency(pendingPeople)}
            </p>
          </div>
        </div>
      ) : (
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {size === "compact" ? (
                <div className="mt-2 flex items-baseline justify-between gap-3">
                  <p className="text-xs font-semibold text-slate-600">{monthlyLabel}</p>
                  <p className="text-base font-semibold text-slate-900">{formatCurrency(monthlyAmount)}</p>
                </div>
              ) : (
